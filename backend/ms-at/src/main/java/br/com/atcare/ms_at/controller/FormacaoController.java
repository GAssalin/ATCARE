...
public class FormacaoController {
    ...
    @PostMapping("/ats/{atId}/formacoes")
    public ResponseEntity<FormacaoResponseDTO> criar(@PathVariable Long atId, @Valid @RequestBody FormacaoRequestDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.criar(atId, dto));
    }

    @GetMapping("/formacoes")
    public ResponseEntity<List<FormacaoResponseDTO>> listarTodos() {
        return ResponseEntity.ok(service.listarTodos());
    }

    @GetMapping("/formacoes/{id}")
    public ResponseEntity<FormacaoResponseDTO> buscarPorId(@PathVariable Long id) {
        return ResponseEntity.ok(service.buscarPorId(id));
    }

    @PutMapping("/formacoes/{id}")
    public ResponseEntity<FormacaoResponseDTO> atualizar(@PathVariable Long id, @Valid @RequestBody FormacaoRequestDTO dto) {
    ...
    @PatchMapping("/formacoes/{id}/ativar")
    public ResponseEntity<Void> ativar(@PathVariable Long id) {
        service.ativar(id);
        return ResponseEntity.noContent().build();
    }

    @PatchMapping("/formacoes/{id}/inativar")
