...
public class CursoController {
    ...
    @PostMapping("/formacoes/{formacaoId}/cursos")
    public ResponseEntity<CursoResponseDTO> criar(@PathVariable Long formacaoId, @Valid @RequestBody CursoRequestDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.criar(formacaoId, dto));
    }
    ...
    @PatchMapping("/cursos/{id}/ativar")
    public ResponseEntity<Void> ativar(@PathVariable Long id) {
        service.ativar(id);
        return ResponseEntity.noContent().build();
    }

    @PatchMapping("/cursos/{id}/inativar")
