...
import br.com.atcare.ms_at.mapper.CertificadoMapper;
...
public class CertificadoService {
    ...
    public CertificadoResponseDTO criar(Long cursoId, CertificadoRequestDTO dto) {
        return mapper.toResponse(repository.save(mapper.toEntity(dto, buscarCurso(cursoId))));
    }
    ...
    public CertificadoResponseDTO atualizar(Long id, CertificadoRequestDTO dto) {
        Certificado existente = buscarEntidadePorId(id);
        Certificado dados = mapper.toEntity(dto, existente.getCurso());
        existente.setArquivo(dados.getArquivo());
        existente.setValidacoes(dados.getValidacoes());
    ...
    public Certificado buscarEntidadePorId(Long id) {
        return repository.findById(id).orElseThrow(() -> new EntityNotFoundException("Certificado não encontrado. Id: " + id));
    }

    private Curso buscarCurso(Long id) {
        return cursoRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("Curso não encontrado. Id: " + id));
    }

    private void alterarAtivo(Long id, boolean ativo) {
        Certificado entity = buscarEntidadePorId(id);
