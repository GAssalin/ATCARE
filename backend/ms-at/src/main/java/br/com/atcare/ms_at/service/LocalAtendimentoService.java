...
import br.com.atcare.ms_at.mapper.LocalAtendimentoMapper;
...
public class LocalAtendimentoService {
    ...
    private final LocalAtendimentoMapper mapper;
    ...
    public List<LocalAtendimentoResponseDTO> listarTodos() {
        return repository.findAll().stream().map(mapper::toResponse).toList();
    }

    public LocalAtendimentoResponseDTO buscarPorId(Long id) {
    ...
    public LocalAtendimentoResponseDTO atualizar(Long id, LocalAtendimentoRequestDTO dto) {
        LocalAtendimento existente = buscarEntidadePorId(id);
        LocalAtendimento dados = mapper.toEntity(dto, existente.getAt());
        Endereco endereco = dados.getEndereco() == null ? null : enderecoRepository.save(dados.getEndereco());
        existente.setEndereco(endereco);
        existente.setAtendimentoDomiciliar(dados.getAtendimentoDomiciliar());
    ...
    public LocalAtendimento buscarEntidadePorId(Long id) {
        return repository.findById(id).orElseThrow(() -> new EntityNotFoundException("Local de atendimento não encontrado. Id: " + id));
    }
    ...
    private void alterarAtivo(Long id, boolean ativo) {
        LocalAtendimento entity = buscarEntidadePorId(id);
