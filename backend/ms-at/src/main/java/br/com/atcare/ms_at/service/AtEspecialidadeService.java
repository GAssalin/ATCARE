...
import br.com.atcare.ms_at.model.mapper.AtEspecialidadeMapper;
...
public class AtEspecialidadeService {
    private final AtEspecialidadeRepository repository; private final AtRepository atRepository;
    private final EspecialidadeRepository especialidadeRepository; private final AtEspecialidadeMapper mapper;
    ...
    public List<AtEspecialidadeResponseDTO> listarTodos() { return repository.findAll().stream().map(mapper::toResponse).toList(); }
    public AtEspecialidadeResponseDTO buscarPorId(Long id) { return mapper.toResponse(buscarEntidadePorId(id)); }
    public AtEspecialidadeResponseDTO atualizar(Long id, AtEspecialidadeRequestDTO dto) {
        AtEspecialidade existente = buscarEntidadePorId(id); existente.setEspecialidade(buscarEspecialidade(dto.especialidadeId()));
    ...
    public AtEspecialidade buscarEntidadePorId(Long id) { return repository.findById(id).orElseThrow(() -> new EntityNotFoundException("Especialidade do AT não encontrada. Id: " + id)); }
    ...
    private Especialidade buscarEspecialidade(Long id) { return especialidadeRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("Especialidade não encontrada. Id: " + id)); }
    private void alterarAtivo(Long id, boolean ativo) { AtEspecialidade entity = buscarEntidadePorId(id); entity.setAtivo(ativo); repository.save(entity); }
}
