...
import br.com.atcare.ms_at.model.mapper.FormacaoMapper;
...
public class FormacaoService {
    ...
    public FormacaoResponseDTO criar(Long atId, FormacaoRequestDTO dto) { return mapper.toResponse(repository.save(mapper.toEntity(dto, buscarAt(atId)))); }
    public List<FormacaoResponseDTO> listarTodos() { return repository.findAll().stream().map(mapper::toResponse).toList(); }
    public FormacaoResponseDTO buscarPorId(Long id) { return mapper.toResponse(buscarEntidadePorId(id)); }
    public FormacaoResponseDTO atualizar(Long id, FormacaoRequestDTO dto) {
        Formacao existente = buscarEntidadePorId(id); Formacao dados = mapper.toEntity(dto, existente.getAt());
        existente.setInstituicao(dados.getInstituicao()); existente.setTitulo(dados.getTitulo());
        existente.setAnoConclusao(dados.getAnoConclusao()); existente.setCursos(dados.getCursos());
    ...
    public Formacao buscarEntidadePorId(Long id) { return repository.findById(id).orElseThrow(() -> new EntityNotFoundException("Formação não encontrada. Id: " + id)); }
    ...
    private void alterarAtivo(Long id, boolean ativo) { Formacao entity = buscarEntidadePorId(id); entity.setAtivo(ativo); repository.save(entity); }
}
